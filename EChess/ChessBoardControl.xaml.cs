...
namespace SrcChess2 {
    ...
    public partial class ChessBoardControl : UserControl, SearchEngine.ITrace, IXmlSerializable {
        ...
        private void mi_Click(object sender, RoutedEventArgs e)
        {
            ...
            RaiseComment("Test Comment", "This is a test comment " + t.ToString());
        }
        #endregion
        ...
        private void option_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (parent != null)
            {
                parent.ChooseBoardSetting();
            }
        }

        private void toImg_Click(object sender, RoutedEventArgs e)
        {
            SaveAsPicture();
        }
        ContextMenu mnu;
        private void toAvi_Click(object sender, RoutedEventArgs e)
        {
            ExportToAvi();
        }

        public void ExportToAvi()
        {
            //mnu.StaysOpen = false;
            //RenderBmps(10);
            DependencyObject parentObject = Window.GetWindow(this);
            ...
            if (parent != null)
            {
                CreateVideo(parent.m_moveViewer.listViewMoveList.Items.Count);
            }
        }
        /// <summary>
        /// Bảng hiển thị bình luận
        /// </summary>
        //public CommentBoad CommentBoard { get; set; }
        /// <summary>
        /// Hiển thị bình luận
        /// </summary>
        ...
        public void RaiseComment(string title, string content)
        {
            ...
            {
                CommentBoard.ClearComment();
            }
            //CommentBoard = new CommentBoad() { Opacity = 0.85};

            //Border b = new Border() { BorderThickness = new Thickness(0, 2, 0, 2), BorderBrush = Brushes.Red};
            //CommentBoard.Children.Add(b);
            //CellContainer.Children.Add(CommentBoard);
            //Grid.SetRow(CommentBoard, 1);
            //Grid.SetColumnSpan(CommentBoard, 6);
            //Grid.SetRowSpan(CommentBoard, 6);

            CommentBoard.RaiseComment(title, content);
            // Update
            RenderArea.UpdateLayout();
            RenderArea.InvalidateVisual();
        }
        private void ClearComments()
        {
            CellContainer.Children.Remove(CommentBoard);
        }

        public void Test2()
        {
            Model m = new Model();
            m.RenderVisaulToBitmap(RenderArea, (int)RenderArea.ActualWidth, (int)RenderArea.ActualHeight);
        }
        /// <summary>
        /// Lưu Bàn cờ hiện tại dưới dạng file ảnh
        /// </summary>
        public void SaveAsPicture()
        {
            Model m = new Model();
            MemoryStream memstream = m.GenerateImage(RenderArea, (int)RenderArea.ActualWidth, (int)RenderArea.ActualHeight, ImageFormat.BMP);
            ...
            if (memstream != null)
            {
                SaveFileDialog fdlg = new SaveFileDialog
                {
                    DefaultExt = "BMP",
                    Title = "Save As",
                    Filter = "JPEG(*.JPG;*.JPEG;*.JPE)|.JPG|BMP(*.BMP;*.RLE;*.DIB)|*.BMP|PNG(*.PNG)|*.PNG|TIFF(*.TIFF)|*.TIF|GIF(*.GIF)|*.GIF",
                    FilterIndex = 2,
                    FileName = "Untitled",
                    AddExtension = true,
                };
        ...
        Model m= null;
        public void RenderWithFilename(string fName)
        {
            MemoryStream memstream = m.GenerateImage(RenderArea, (int)RenderArea.ActualWidth, (int)RenderArea.ActualHeight, ImageFormat.BMP);
            if (memstream != null)
            {
                    using (FileStream fstream = File.OpenWrite(fName))
                    {
                        memstream.WriteTo(fstream);
                        fstream.Flush();
                        fstream.Close();
        ...
        public void RenderWithoutFilename()
        {
            MemoryStream memstream = m.GenerateImage(RenderArea, (int)RenderArea.ActualWidth, (int)RenderArea.ActualHeight, ImageFormat.BMP);
        }
        private int Step = 3; // Giây, khoảng thời gian giữa mỗi nước đi
